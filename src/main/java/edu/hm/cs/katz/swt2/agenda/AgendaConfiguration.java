...
public class AgendaConfiguration extends WebSecurityConfigurerAdapter {
  ...
  protected void configure(HttpSecurity http) throws Exception {
    http.authorizeRequests()
        ...
        .antMatchers("/assets/**", "/", "/login**", "/logout**", "/registration").permitAll()
        // Administratorzugriff auf Datenbank
        .antMatchers("/h2-console/**").hasRole("ADMIN")
        // Authentifizierung Voraussetzung für alles andere
        .anyRequest().fullyAuthenticated()
        // Login-Handling
        .and().formLogin().loginPage("/login").defaultSuccessUrl("/tasks", true).and().logout().logoutSuccessUrl("/login").permitAll();
